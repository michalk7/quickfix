$(document).ready(function() {
	...
	$(".delete-expert-button").each(function() {
		
		$(this).click(function() {
			let deleteUrl = $(this).attr("data-href");
			
			 swalWithBootstrapButtons.fire({
 				  title: 'Czy na pewno chcesz usunąć tego fachowca z obsługi tego osiedla?',
 				  icon: 'question',
 				  text: 'Akcja ta jest nieodwracalna.',
 				  confirmButtonText: 'Usuń',
 				  showCancelButton: true,
 						 ...
 						 success: function(response) {
 							 ...
 							 if(!response.error) {
 								 swalWithBootstrapButtons.fire({
 									title: 'Gotowe!',
 									icon: 'success',
 									text: 'Fachowiec nie obsługuje już tego osiedla. W ciągu 5 sekund nastapi odświeżenie strony.',
 									timer: 5000,
 									allowOutsideClick: false,
 									allowEscapeKey: false
 								 }).then((resultT) => {
